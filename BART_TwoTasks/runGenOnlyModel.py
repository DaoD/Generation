...
from BARTGenOnlyModel import GenOnlyModel
...
parser.add_argument("--model_name_or_path", default="/data00/yutao_zhu/pretrained_models/bartbase/", type=str, help="")
parser.add_argument("--config_name", default="/data00/yutao_zhu/pretrained_models/bartbase/", type=str, help="")
parser.add_argument("--tokenizer_name", default="/data00/yutao_zhu/pretrained_models/bartbase/", type=str, help="")
parser.add_argument("--data_dir", default="../data/reddit3/", type=str, help="")
...
parser.add_argument("--per_gpu_train_batch_size", default=256, type=int, help="")
parser.add_argument("--per_gpu_eval_batch_size", default=256, type=int, help="")
...
parser.add_argument("--logging_steps", default=2000, type=int, help="")
...
def train_model():
    ...
    model = GenOnlyModel(bart, config)
...
def fit(model, X_train, X_test):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.zero_grad()
            global_step += 1
            epoch_iterator.set_postfix(lr=scheduler.get_last_lr()[0], ce=loss.item())
            # if step > 0 and step % args.logging_steps == 0:
            #     tqdm.write("Step = {:d}\tLR = {:.6f}\tTotal Loss = {:.6f}\tGen Loss = {:.6f}\tRet Loss = {:.6f}".format(step, scheduler.get_last_lr()[0], (total_loss - tmp_loss) / args.logging_steps, (total_gen_loss - tmp_gen_loss) / args.logging_steps, (total_ret_loss - tmp_ret_loss) / args.logging_steps))
            #     tmp_loss = total_loss
            #     tmp_gen_loss = total_gen_loss
            #     tmp_ret_loss = total_ret_loss
            if step > 0 and step % args.save_steps == 0:
                tqdm.write("Step = {:d}\tLR = {:.6f}\tTotal Loss = {:.6f}".format(step, scheduler.get_last_lr()[0], (total_loss - tmp_loss) / args.save_steps))
                tmp_loss = total_loss
            ...
            if args.max_steps > 0 and global_step > args.max_steps:
                break
        tqdm.write("Epoch = {:d}\tLoss = {:.6f}".format(epoch + 1, total_loss / len(train_dataloader)))
        if args.max_steps > 0 and global_step > args.max_steps:
            break
...
def evaluate(model, X_test, best_result):
    ...
    if all_test_loss < best_result:
        ...
        tqdm.write("Best Test Loss = {:.6f}, Perplexity = {:.6f}".format(all_test_loss, perplexity.item()))
...
def test_model_generation():
    ...
    bart = BartForConditionalGeneration.from_pretrained(args.model_name_or_path, config=config)
    model = GenOnlyModel(bart, config)
    model_state_dict = torch.load("../output/models/bart/genonly.pt")
    ...
    with open(args.result_dir + "test.result.bartgenonly.greedy.txt", "w", encoding="utf-8") as fw:
