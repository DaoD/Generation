...
def train_model():
    ...
    bart_ctx = BartForConditionalGeneration.from_pretrained(args.model_name_or_path, config=config)
    model = BartEnsembleEmbeddingModel(bart_ctx, pre_trained_bart.bart_rep, config)
...
def evaluate(model, X_test, best_result):
    ...
    if all_test_loss < best_result:
        ...
        torch.save(model_to_save.state_dict(), args.output_dir + "ensemble.bienc.embedding.pt")
    return best_result
