...
class BartGAR(nn.Module):
    def __init__(self, bart, config):
        ...
        self.config = config
        # for classification
        self.pooler = BartClassificationHead(config.d_model, config.d_model, 1, 0.1)
    ...
    def forward(self, batch_data, is_test=False):
        ...
        sequence_outputs = self.bart_conditional.model(input_ids=sequence, attention_mask=sequence_attn_mask, decoder_input_ids=classification_head_token)
        sequence_hidden = sequence_outputs.encoder_last_hidden_state 
        sequence_score = self.pooler(sequence_hidden[:, 0, :]).squeeze(1)
