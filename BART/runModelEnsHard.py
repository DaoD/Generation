...
def evaluate(model, X_test, best_result):
    ...
    if all_test_loss < best_result:
        ...
        torch.save(model_to_save.state_dict(), args.output_dir + "ensemble.hard_neg.pt")
    return best_result

def test_model_generation():
    test_data = "../data/" + args.dataset + "/train.20w.txt"
    config = BartConfig.from_pretrained(args.config_name)
    ...
    # input_context = "if they stopped selling plastic bottles then people would bring their own bottles as like everyone takes water to the park no ? how about selling non plastic bottles ? did that ever occur to you ?"
    ...
    # seq_outputs = bart(input_ids=torch.tensor([context_context_input_ids]), attention_mask=torch.tensor([context_attention_mask]))
    # lm_logits = seq_outputs.logits

    # print(lm_logits.size())
    # print(lm_logits.argmax(dim=-1)[0, -1])
    # print(tokenizer.decode(lm_logits.argmax(dim=-1)[0, -1]))
    ...
    # output_ids = model.bart_conditional.generate(inputs.input_ids)
    # print(tokenizer.batch_decode(output_ids, skip_special_tokens=False, clean_up_tokenization_spaces=False))
    ...
    with open(args.result_dir + "train.20w.prediction.bart.greedy.txt", "w", encoding="utf-8") as fw:
...
def encoding_text():
    test_data = "../data/" + args.dataset + "/test.rep.dict.txt"
    config = BartConfig.from_pretrained(args.config_name)
    bart = BartForConditionalGeneration.from_pretrained(args.model_name_or_path, config=config)
    model = EnsembleModel(bart, config)
    model_state_dict = torch.load("../output/models/bart/ensemble.hard_neg.pt")
    model.load_state_dict(model_state_dict, strict=True)
    model = model.to(device)
    model = torch.nn.DataParallel(model)
    model.eval()
    ...
    torch.save(all_test_rep, args.output_dir + "test.rep.ensemble.hard_neg.pt", pickle_protocol=4)
