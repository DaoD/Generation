...
def evaluate(model, X_test, best_result):
    ...
    if all_test_loss < best_result:
        ...
        torch.save(model_to_save.state_dict(), args.output_dir + "ensemble.pt")
    return best_result
...
def test_model_generation():
    ...
    bart = BartForConditionalGeneration.from_pretrained(args.model_name_or_path, config=config)
    model = EnsembleModel(bart, config)
    model_state_dict = torch.load("../output/models/bart/ensemble.pt", map_location=torch.device('cpu'))
    ...
    # with open(args.result_dir + "result.bart.ensemble.txt", "w", encoding="utf-8") as fw:
...
def encoding_text():
    ...
    torch.save(all_test_rep, args.output_dir + "test.10w.bart.rep.onlyret.pt")
