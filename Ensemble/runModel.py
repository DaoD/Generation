...
parser.add_argument("--per_gpu_train_batch_size", default=32, type=int, help="")
parser.add_argument("--per_gpu_eval_batch_size", default=32, type=int, help="")
...
def train_model():
    ...
    model = BartEnsembleEmbeddingModel(bart_encdec, config)
...
def evaluate(model, X_test, best_result):
    ...
    if all_test_loss < best_result:
        ...
        torch.save(model_to_save.state_dict(), args.output_dir + "ensemble.embedding.pt")
    return best_result
...
def encoding_text():
    config = BartConfig.from_pretrained(args.config_name)
    bart = BartForConditionalGeneration.from_pretrained(args.model_name_or_path, config=config)
    model = BartEnsembleEmbeddingModel(bart, config)
